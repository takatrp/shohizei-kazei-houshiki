...
test('リリースメタデータを最新版から一元生成する', () => {
  ...
  assert.equal(release.APP_META.version, 'r34');
  assert.equal(release.RELEASE_HISTORY[0].recalcRecommended,false);
  const previous = release.RELEASE_HISTORY.slice(1);
  assert.equal(previous[0].version,'r33');
  assert.equal(previous[0].recalcRecommended,true);
  assert.equal(previous[1].version,'r32.2');
  assert.equal(previous[1].recalcRecommended,true);
  assert.equal(previous[2].version,'r32.1');
  assert.equal(previous[3].version,'r32');
  assert.equal(previous[3].recalcRecommended,false);
  assert.equal(previous[4].version,'r31.1');
  assert.equal(previous[4].recalcRecommended,true);
  assert.equal(previous[5].version,'r31');
  assert.equal(previous[5].recalcRecommended,false);
  assert.equal(previous[6].version,'r30');
  assert.equal(previous[6].recalcRecommended,true);
