...
for(const file of ['tax-engine.js', 'journal-csv.js', 'release-history.js', 'forstaff.png']){
  if(!fs.existsSync(path.join(root, file))) throw new Error(`Missing static asset: ${file}`);
}

for(const file of ['tax-engine.js', 'journal-csv.js', 'release-history.js']){
...
for(const marker of ['release-history.js', 'tax-engine.js', 'journal-csv.js', 'journalCsvFile', 'latestRelease', 'methodCards', 'projectionRows']){
